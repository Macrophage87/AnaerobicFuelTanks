...
budgets a data field 32~bytes of developer fields per message type, shared with any other installed
fields, so the per-system consumption and depleted totals are derived from the reserve streams
($\max(0, -\Delta R)$ and its running sum) rather than recorded. The state is a
