...
\paragraph{Optional realism terms.} Two of the three ship on by default and are included in every headline
number here---the pH-slowed PCr recovery ($\mathrm{fatK} = 0.75$) and the aerobic ramp
($\tauaer = 25~\mathrm{s}$); the glycolytic flux-fatigue term ($g_{\mathrm{fat}} = 0$) ships off. Each can
be disabled by zeroing its coefficient.
\begin{itemize}[leftmargin=1.4em,itemsep=1pt]
\item \emph{pH-slowed PCr recovery} (on): $\tau_{\mathrm{p,eff}} = \taup\,(1 + k(1 - \Rg/\Cg))$. It cannot be
...
\item \emph{Aerobic ramp} ($\tauaer \approx 25~\mathrm{s}$, on): a first-order aerobic supply $A(t)$ rising
...
$\tauon = \tauoff = 6$, $\LTone = 204~\mathrm{W}$, with the shipped optional terms $\mathrm{fatK} = 0.75$
and $\tauaer = 25~\mathrm{s}$ on and $g_{\mathrm{fat}} = 0$ off. \Cref{tab:recovery} shows the recovery law's behaviour
...
fixed fraction (\cref{sec:assumptions}), and is labelled as such or omitted. The two reserves---and only
those---are written to the FIT file via \texttt{FitContributor} fields for post-ride analysis; Connect~IQ
budgets a data field 32~bytes of developer fields per message type; to keep the field's contribution
to the ride file small, the per-system consumption and depleted totals are derived from the reserve streams ($\max(0, -\Delta R)/\Delta t$ for the
per-sample draw in watts, and $\sum \max(0, -\Delta R)$ for the joules) rather than recorded. The state is a
