...
- **What the field stops recording.** Seven developer fields become **two**: `PCr_J` and `GLY_J`
  ...
  IQ's 32 bytes per message type for a data field — a budget shared with whatever else the athlete
  has installed, which is the reported failure mode this addresses. A `fitRecord` setting (**on by
...
- **What is lost, stated as a loss.** The draws and the depleted totals are reconstructible from the
  reserve streams as `max(0, −ΔR)` and its running sum, but **only between out-of-band reserve
