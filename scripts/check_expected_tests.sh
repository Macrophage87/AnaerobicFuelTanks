...
# actually exist in the Monkey C sources under connectiq/source/.
#
...
if [ "$n_source" -eq 0 ]; then
    echo "::error::no (:test) functions found under connectiq/source/ -- the extractor or"
...
if ! diff -u <(printf '%s\n' "$from_pin") <(printf '%s\n' "$from_source") \
        --label "$PIN (pinned)" --label "connectiq/source/**.mc (actual)"; then
    echo "::error::$PIN has drifted from the (:test) functions under connectiq/source/."
...
echo "OK: $n_source (:test) function(s) under connectiq/source/ match $PIN exactly."
