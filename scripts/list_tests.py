#!/usr/bin/env python3
"""List the (:test) function names declared under connectiq/source/, comment-aware.
...
# a -t (test) build of this repository for fenix6pro fails, once enough
# file-scope declarations exist, with
#
...
# drop N throwaway file-scope (:test) functions into connectiq/source/, compile
# for fenix6pro with -t, and read N back out of the reported total; then confirm
# by bisection that N=free builds and N=free+1 reds. The measured line lives in
# ONE place in source -- the header of connectiq/source/Tests.mc -- and is quoted
# by docs/agents/FACTS.md in the machine-checked AGENTFACT ceiling line.
# scripts/check_ceiling_notes.py checks the arithmetic of every copy and that
# copies sharing an anchor are identical; scripts/check_agent_facts.py checks
# that FACTS.md quotes the copy in the tree. No figure is repeated here, because
# a repeated figure is a second thing to get wrong.
#
# THE LIMIT IS INCLUSIVE: at F free, the F-th added file-scope (:test) still
# BUILDS and the (F+1)-th is the first that reds. Bisected here on fenix6pro at
# 30b2b99 (2026-09-05): N=226 BUILD SUCCESSFUL, N=227 "Found 254 members in
# module 'globals', exceeding the limit of 253". The error names neither the
# test nor the file, and edge1050 (the other CI compile target) stays green --
# so a green edge1050 build proves nothing about fenix6pro.
#
...
def main():
    ...
    ap.add_argument("--root", default="connectiq/source")
    args = ap.parse_args()
