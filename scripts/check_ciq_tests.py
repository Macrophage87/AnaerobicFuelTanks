...
# one, and the simulator prints its row module-qualified:
#
#     M.test_foo                                           PASS
#
# A pattern that could not match that line would silently DROP the row -- and a
# dropped row does not produce a clear error, it produces the "executed but
# mis-tallied" complaint below, which names the wrong cause.
#
# INERT IN THIS REPOSITORY TODAY, AND SAID SO ON PURPOSE. `connectiq/source/`
# declares no `module { }` block: verified at cea95c8, every occurrence of the
# word `module` in the four .mc files is inside a comment. Every name in
# scripts/expected_tests.txt is bare, so no dotted row can occur here yet. The
# dotted form is NOT a measurement taken on this app -- it is carried over with
# the parser, and it is kept because scripts/list_tests.py already emits
# module-qualified names for that case, so dropping it here would deadlock the
# pin against the parser the day a module appears. Module scope is also the
# only way to add a (:test) without spending a fenix6pro `globals` member
# (FACTS.md 5.1), so that day is foreseeable.
#
...
def parse_results_table(text):
    ...
    Why the parse is scoped to the block, stated against the committed capture
    rather than from memory: a whole-file scan does NOT double-count every test.
    In scripts/fixtures/monkeydo-green-run.log the simulator puts
    `Executing test <name>...` and `PASS` on SEPARATE lines, so neither half of
    the preamble is row-shaped and a whole-file scan of that capture yields 18
    row matches, not 34 -- the 17 real rows plus the `Test: Status:` header.
...
def main():
    ...
    ap.add_argument("--harness-status", default=None,
                    help="optional key=value breadcrumb file from a harness "
                         "wrapper, folded into the report. No such wrapper is "
                         "committed here and the ciq-test job does not pass it; "
                         "the path is kept so a degraded run can explain itself "
                         "rather than being guessed at (see the empty-log branch)")
    args = ap.parse_args()
    ...
    # --- Test-name set -------------------------------------------------------
    # THE INVARIANT THAT MAKES THIS SAFE (measured here, not designed): monkeydo
    # emits the complete RESULTS block -- header + rows + tally + summary -- to
    # its OWN stdout as one contiguous block. Measured on every ciq-test run
    # this repository has that executed the suite, all on 2026-09-05:
    # 33990588668 (16 PASS), 33990860226 (one ERROR), 33991598740 (17 PASS) --
    # in each, the `cat /tmp/ciq-test.log` echo shows the block unbroken. Two of
    # those three are committed under scripts/fixtures/. A table split across
    # the two log files has never been observed here. monkeydo's table is
    # authoritative. Do not extend the machinery below to "recover" exotic table
    # shapes -- fail them.
    #
    # N = 3 runs on one branch on one day. That is enough to pin the SHAPE the
    # parser reads; it is not evidence that the job runs the suite reliably, and
    # nothing here should be cited for that (#61).
    #
