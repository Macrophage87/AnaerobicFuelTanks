...
#   * CP, W', pPmax  -> from the combined mean-maximal power curve (best-of-all).
#   * fP, tauP, tauG, eta -> fit the dual-tank model on each variable-power ride
#     (race / hard group ride / intermittent test) by anchoring "reserve = 0" at
#     maximal / cracked moments, then aggregate across rides (median or best-fit).
#
# IDENTIFIABILITY: a single maximal effort obeys t_lim = W'/(P-CP) -> only CP/W'.
# The split and recovery rates appear only across repeated efforts with recovery,
# and only when a ride has "reserve=0" anchors (final sprint, the attack you
# couldn't follow, the moment you got dropped).
#
...
mmp_curve <- function(power_list, durations = DURATIONS)   # best per duration across files
...
# fit every ride (auto-anchored) and return a per-ride table
fit_all_rides <- function(power_list, names, cp, base) {
  rows <- lapply(seq_along(power_list), function(i) {
    p <- power_list[[i]]; marks <- suggest_marks(p, cp, base)
    r <- try(fit_recovery(p, cp, base, marks), silent = TRUE)
    if (inherits(r, "try-error")) return(NULL)
    data.frame(file = names[i], fP = r$fP, tauP = r$tauP, tauG = r$tauG,
               eta = r$eta, obj = r$obj, conv = r$conv, anchors = r$n)
  })
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      hr(),
      h5("Recovery fit across all rides"),
      ...
      helpText("Each ride is auto-anchored at its lowest-reserve points; rides",
               "with clear maximal/cracked moments (races, hard group rides) fit best.")
    ),
...
server <- function(input, output, session) {
  ...
  powers <- reactive({
    ...
    keep <- !vapply(ps, is.null, logical(1))
  ...
  # aggregate recovery params across rides
  agg <- reactive({
    df <- rv$fits; if (is.null(df)) return(NULL)
    ok <- df[df$conv == 0, , drop = FALSE]; if (!nrow(ok)) ok <- df
    if (input$agg == "best-fit") {
      b <- ok[which.min(ok$obj), ]; list(fP = b$fP, tauP = b$tauP, tauG = b$tauG, eta = b$eta, src = paste("best-fit:", b$file))
    } else {
      list(fP = median(ok$fP), tauP = median(ok$tauP), tauG = median(ok$tauG),
  ...
  observeEvent(input$fitall, {
    ...
    rv$fits <- fit_all_rides(pw$p, pw$names, f$CP, base_par())
  })

  output$fit_tbl <- DT::renderDataTable({
    validate(need(!is.null(rv$fits), "Click 'Fit ... on every ride'."))
    d <- rv$fits; d[c("fP","tauP","tauG","eta","obj")] <- lapply(d[c("fP","tauP","tauG","eta","obj")],
                                                                 function(x) round(x, 3))
    DT::datatable(d, rownames = FALSE, options = list(pageLength = 10, dom = "tp"))
  })
  output$agg_txt <- renderText({
    a <- agg(); if (is.null(a)) return("")
    sprintf("Chosen (%s):  fP = %.2f   tauP = %.0f s   tauG = %.0f s   eta = %.2f",
            a$src, a$fP, a$tauP, a$tauG, a$eta)
  })
  ...
  output$caveats <- renderUI(HTML(paste0(
    "<b>CP, W', pPmax</b> take the best value across every uploaded file (combined ",
    "mean-maximal curve). <b>fP, tauP, tauG, eta</b> are fit on each ride that has ",
    "maximal/cracked anchors, then combined by median or best-fit. To separate fast ",
    "(PCr) from slow (glycolytic) recovery, include rides whose hard efforts had ",
    "<i>different</i> recovery lead-ins. A race or hard group ride works well &mdash; ",
    "getting dropped is a natural reserve=0 anchor. <b>lt1Frac, fatK, tauAer</b> stay ",
    "at defaults (need a threshold / repeated-bout protocol).")))
}
