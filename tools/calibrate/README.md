...
- The UI uses a standard offline `bslib` theme (`bs_theme(version = 5, …)` in `app.R`) — no web
  fonts and no startup network dependency; it runs fully offline.
...
The committed `manifest.json` is a **complete, version-pinned dependency closure** (the full
transitive set of packages with exact versions and per-file md5s, generated by
`rsconnect::writeManifest()`). Regenerating it against your own R library **refreshes the pins and
`platform`** to match your environment — see `deploy.R`:
