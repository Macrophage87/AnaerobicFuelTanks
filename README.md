...
- **records to the FIT file** — two streams and nothing else: per-second `PCr_J` / `GLY_J` reserve
  ...
  **on by default**, and a change to it applies the next time the field loads. The streams are also
  ...
  never chose. That too is decided once at load, so setting CP/W′ after the field has loaded — mid-ride, or on the
  pre-ride screen after seeing "SET CP/W'" — records nothing until the next load (start a new ride), and clearing them mid-ride keeps the streams that load already created. Issue #102 pruned the
...
- **Compiled in CI on two devices; the `(:test)` suite executes on one.** `.github/workflows/ci.yml`
  builds the Monkey C with `monkeyc -t -l 1` (Gradual) on `edge1050` and `fenix6pro` in the required
  `test` job, and has since PR #48 (2026-07-16). The required `ciq-test` job runs the `(:test)` suite
  in a headless simulator on `edge1050` and fails unless every pinned case passes (#61); on any other
  device a local `monkeydo` run is the only execution. Strict `-l 3` type checking is **not** run in CI, nor by
  ...
  manifest devices, its unit tests pass on one, and the model arithmetic agrees; it does not mean the field loads or records on
